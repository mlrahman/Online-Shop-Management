...
?>	
	...
	<?php
		...
		$query="select * from product where name LIKE '%$search_value%' AND status='active'  AND seller_id='$seller_id' order by name asc ";
	...
	?>
				<div id="product_view" style="margin-left:35px;" onclick="open_details(<?php echo $data['product_id']; ?>)" title="Click for details of this product.">
					<?php 
	...
	<?php
		...
		if($cou==0)
		{
			echo '<h1 style="color:red;text-align:center;font-size:15px;margin-top:10px;">Oops no products available!!!</h1>';
		}
	?>
